...
int16 CFont::Size[LANGSET_MAX][MAX_FONTS][242] = {
	{
#else
int16 CFont::Size[MAX_FONTS][210] = {
	...
	},
	{
		...
		12, 14, 11, 11, 16, 11, 12, 14, 14, 10, 13, 12, 10, 19, 18, 12,
		16, 13, 13, 11, 12, 15, 12, 15, 13, 12, 12, 37, 33, 37, 35, 37,
		17, 20, 19, 18, 19, 18, 28, 20, 20, 20, 17, 19, 30, 23, 19, 22,
		22, 16, 18, 18, 23, 19, 18, 20, 30, 28, 18, 24, 16, 22, 29, 21,
		14, 12, 11,  9, 14, 10, 24, 12, 14, 14, 12, 13, 16, 13, 11, 16,
		15, 11, 18, 12, 19, 13, 13, 11, 17, 17, 11, 15,  7,  9, 15, 13,
		19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19,
		19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19,
		19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19,
		19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19,
		...
		10, 17, 17, 16, 17, 17, 11, 17, 17,  7,  7, 18,  7, 25, 17, 17,
		17, 17, 11, 17, 11, 17, 18, 25, 19, 18, 17, 28, 26, 20, 15, 15,
		23, 17, 16, 13, 25, 14, 27, 16, 20, 20, 17, 22, 24, 18, 24, 18,
		16, 22, 14, 20, 25, 20, 20, 17, 26, 29, 20, 22, 17, 22, 31, 17,
		18, 19, 14, 12, 20, 18, 24, 14, 16, 17, 16, 18, 21, 16, 19, 15,
		19, 18, 13, 18, 23, 17, 18, 15, 24, 25, 18, 20, 14, 18, 25, 15,
		20, 18, 19, 19, 21, 19, 19, 19, 19, 19, 16,  1,  1, 21, 10,  9,
		15, 19, 19, 19, 20, 19, 16, 19, 19,  9, 19, 20, 14, 29, 19, 19,
		19, 19, 19, 19, 21, 19, 20, 32, 20, 19, 19,  1,  1,  1,  1,  1,
		19, 19, 19, 19, 19, 19, 29, 19, 19, 19, 20, 22, 31, 19, 19, 19,
		19, 19, 29, 19, 29, 19, 21, 19, 30, 31, 21, 29, 19, 19, 29, 19,
		10, 20
		},
	},	
	{
...
CFont::PrintChar(float x, float y, wchar c)
{
	...
	#ifdef MORE_LANGUAGES
	if (Details.bFontHalfTexture && c == 208 && LanguageSet == FONT_LANGSET_EFIGS)
	c = '\0';
	#else
		...
		if (RenderState.slant == 0.0f) {
			{
				CSprite2d::AddToBuffer(
					...
					RenderState.color,
					xoff / 16.0f, yoff / 25.6f + 0.00105f,
					(xoff + 1.0f) / 16.0f - 0.001f, yoff / 25.6f + 0.00105f,
					xoff / 16.0f, (yoff + 1.0f) / 25.6f - 0.00105f,
					(xoff + 1.0f) / 16.0f - 0.001f, (yoff + 1.0f) / 25.6f - 0.00105f);
			}
		} else
			CSprite2d::AddToBuffer(
				CRect(x, y,
					...
					y + 40.0f * RenderState.scaleY * 0.5f),
				RenderState.color,
				xoff / 16.0f, yoff / 25.6f + 0.000275f,
				(xoff + 1.0f) / 16.0f - 0.001f, yoff / 25.6f + 0.0021f + 0.005f,
				xoff / 16.0f, (yoff + 1.0f) / 25.6f - 0.0045f,
				(xoff + 1.0f) / 16.0f - 0.001f, (yoff + 1.0f) / 25.6f - 0.0021f + 0.005f);
...
CFont::RenderFontBuffer()
{
	...
	for (++pRenderStateBufPointer.pRenderState; pRenderStateBufPointer.pStr < FontRenderStatePointer.pStr; pRenderStateBufPointer.pStr++) {
		...
		#ifdef MORE_LANGUAGES
				if (IsRussian())
					c = FindNewRussianCharacter(c); 
				else
					c = FindNewCharacter(c);
				
			else if (c > 159) //russian change
			c = '\0'; 
		#else
			c = FindNewCharacter(c); 
...
#ifdef MORE_LANGUAGES
wchar CFont::FindNewRussianCharacter(wchar c)
{
	if (c >= 16 && c <= 26) return c + 144; // цифры
	if (c >= 8 && c <= 9) return c + 86; // скобки
	if (c == 4) return c + 89; // доллар
	if (c == 7) return 174; // '
	if (c == 14) return 175; // точка
	if (c >= 33 && c <= 58) return c + 144;
	if (c >= 65 && c <= 90) return c + 112;
	if (c >= 96 && c <= 127) return c + 112; // русские заглав
	if (c >= 128 && c <= 159) return c + 80; // русские строч
